...
var gangs = [ "goliath", "escher", "orlock", "vanSaar", "cawdor", "delaque" ]
var types = [ "Gang", "Ganger" ]
...
for (let j=0; j<types.length; j++)
{
	...
	{
		var gangButton = document.getElementById(gangs[i] + types[j] + 'Button');
		gangButton.addEventListener('click', function() {
			...
			buttons[j*gangs.length+i].style.fontWeight = 'bold';
			});
		gangButton.disabled = true;
		buttons.push(gangButton);
	}
}
...
function Generate()
{
	...
	while (currentSegment < segments)
	{
		...
		var chance = chanceSegments[0].indexOf(currentSegment+1)
		if (chance != -1)
		{
			if (Math.random() > chanceSegments[1][chance])
			{
				skip = true;				
			}
		}
		...
		{
			entryIndex = GetRandom(0, database.length-1)
			entry = database[entryIndex][index]
		}
		
		if (doubleSegments.indexOf(currentSegment+1) != -1)
		{
			index++
			if (database[entryIndex][index] == false)
			{
				index++
				var entry2 = null
				while (entry2 == null || entry2 == "")
				{
					entry2 = database[GetRandom(0, database.length-1)][index]
				}
				entry += entry2.toLowerCase()
			}
		}
...
function OnDatabaseLoaded(data)
{
	...
	{
		if (database[0][i] == "Segments:")
		{
			i++
			segments = database[0][i]
		}
		else if (database[0][i] == "DoubleSegment:")
		{
			i++
			doubleSegments.push(database[0][i])
		}
		else if (database[0][i] == "ChanceSegment:")
		{
			i++
			chanceSegments[0].push(database[0][i])
			i++
			chanceSegments[1].push(database[0][i])
		}
		else if (database[0][i] == "PostProcess:")
		{
			i++
			postProcess.push(database[0][i])
		}
		else if (database[0][i] == "OutputPrefix:")
		{
			i++
			outputPrefix = database[0][i]
		}
	}
	...
	console.log(" doubleSegments = " + doubleSegments)
	console.log(" chanceSegments = ")
	console.log(chanceSegments)
...
function DoPostProcess_1337Replacement(name)
{
	var replacements = [
  ['A','4'],
  ['E','3'],
